...
function Header() {
  const {userData}=useContext(Appcreatecontext)
  return (
    <div className='flex flex-col items-center mt-20 px-4 text-center'>
      <img src={assets.header_img} className='w-36 h-36 rounded-full mb-6' alt="" />
      <h1 className='flex items-center gap-2 sm:text-3xl text-xl font-medium mb-2'>Hey {userData?userData.name: "Developer" }<img src={assets.hand_wave} className='w-8 aspect-square' alt="" /></h1>
